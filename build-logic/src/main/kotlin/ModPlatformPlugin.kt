...
abstract class ModPlatformPlugin @Inject constructor() : Plugin<Project> {
	...
	private fun Project.configureJava(ctx: Context) {
		extensions.configure<JavaPluginExtension>("java") {
			...
			withJavadocJar()
			toolchain {
				languageVersion.set(org.gradle.jvm.toolchain.JavaLanguageVersion.of(ctx.javaVersion.majorVersion.toInt()))
			}
		}
	}
	...
	private fun Project.configureJarTask(ctx: Context) {
		...
		tasks.withType<Jar>().configureEach {
			...
			if (ctx.loader is Loader.Forge) {
				manifest.attributes[Loader.Forge.mixinConfigAttribute] = "${ctx.modId}.mixins.json"
			}
		}
	}
	...
	private fun Project.registerBuildAndCollectTask(ctx: Context) {
		tasks.register<Copy>("buildAndCollect") {
			...
			dependsOn("build")
			group = "build"
			description = "Build and copy the main jar (no sources/javadoc) into root build/dist"
		}
	}
}
